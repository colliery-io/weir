...
- Sub-second true stream *computation* — windowing, joins, aggregation over a stream — pair with Kafka/Flink. Our compute cadence stays batch, micro-batch, CDC. **Amended 2026-07-08, re-tightened 2026-08-04 ([[WEIR-A-0040]]):** *long-lived resident sources that stay up and emit* (continuous-polling / event-reader) are **in scope** — weir may **carry** a live signal continuously and land it, but still **never computes over it or decides on it**. "Stay up and emit," not "compute over a stream." **Serving signals *out* to subscribers is out of scope:** the pub/sub delivery plane — socket fan-out to many subscribers, a signal broker, a live-signal registry — was withdrawn ([[WEIR-A-0040]] supersedes [[WEIR-A-0038]]); weir carries a signal *in* and lands it, it is not a subscribable fabric. Retained: F1 resident sources ([[WEIR-I-0035]]) and F3's freshness triple ([[WEIR-I-0037]], re-scoped to the connector contract).
