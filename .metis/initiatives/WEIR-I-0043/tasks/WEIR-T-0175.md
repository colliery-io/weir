...
**2026-08-29 — wire-level MySQL proof (user-requested addendum).** The hostname egress is now proven end-to-end through a REAL wasm guest against real wire bytes, MySQL chosen because the server speaks first (the handshake greeting arrives unsolicited — no MySQL client needed in the guest):

- New fixture `wasm-fixtures/tcp-probe` (fidius 0.5.8, `tcp` capability): dials config's `host:port` over `fidius_guest::sockets::tcp` and emits the server's first bytes hex-encoded as one record; failures (incl. policy denials) surface as transient `Fatal`. Covered by the glob drift guard + sync-contract (12 guests now); NOT staged in production (test-only, like resident).
- New suite `crates/weir-engine/tests/wasm_tcp_probe_engine.rs`. Always-on trio against a local canned-greeting server (byte-faithful MySQL protocol-10 head, bound via `("localhost", 0)` mirroring fidius's own e2e): name-keyed allow-list permits the by-name dial and the greeting decodes (`8.4.0-canned`); a wrong-name list denies the dial; a name-keyed list denies an IP-literal dial to the same address (fail-closed `host: None`). Plus a docker-gated `#[ignore]` test against a genuine mysqld.
- New `mysql:8.4` service in compose's `integration` profile (loopback-only, `WEIR_MYSQL_HOST_PORT` override, mysqladmin healthcheck) — run green live: real greeting probed by hostname through the allow-list (`8.` version string asserted), then torn down.
- Harness gotcha recorded in-file: guest calls must be driven with `futures::executor::block_on`, never inside a tokio runtime (wasmtime-wasi does its own block_on — same rule as the engine's read driver).
- Gate: probe suite 3/3 + ignored 1/1 live, codegen drift 2/2 (12 guests), `angreal check all` clean, `angreal test unit` 12/12.
- Reminder this establishes for [[WEIR-I-0043]]: no MySQL *connector* exists (post-alpha non-goal) — this is egress-policy wire proof only.
