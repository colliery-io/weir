...
/// FORK DELTA (weir-tiberius): the PLP "unknown size" wire sentinel
/// (`0xfffffffffffffffe`), truncating-cast so it compiles portably on 32-bit
/// targets (wasm32) — upstream's bare `_usize` literal only builds there
/// because dependency lint-capping silences the overflow. The truncated value
/// still routes to the PLP decode arm (any value ≥ 0xffff does), and the
/// encode side writes the full u64 constant explicitly.
pub(crate) const PLP_UNKNOWN_SENTINEL: usize = 0xfffffffffffffffe_u64 as usize;
