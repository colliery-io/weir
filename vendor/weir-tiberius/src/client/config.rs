...
pub(crate) enum TrustConfig {
    ...
    /// FORK DELTA (weir-tiberius): an inline-PEM CA bundle — wasm guests have
    /// no filesystem, so path-based trust is unusable there.
...
impl Config {
    ...
    /// FORK DELTA (weir-tiberius): validate the server certificate against the
    /// given inline PEM CA bundle — the filesystem-free counterpart of
    /// [`trust_cert_ca`](Self::trust_cert_ca) for sandboxed (wasm) callers.
    ///
    /// # Panics
    /// Will panic in case `trust_cert` was called before.
    pub fn trust_cert_pem(&mut self, pem: impl ToString) {
        if let TrustConfig::TrustAll = &self.trust {
            panic!("'trust_cert' and 'trust_cert_pem' are mutual exclusive! Only use one.")
        } else {
            self.trust = TrustConfig::CaCertificatePem(pem.to_string())
        }
    }
